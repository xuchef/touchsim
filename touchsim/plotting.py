...
def plot_response(obj, **args):
    ...
    if not spatial:
        ...
        for i in range(len(idx)):
            ...
            spikes[i] = s.options(position=0.1*i,
                                  spike_length=0.1,
                                  color=tuple(Afferent.affcol[obj.aff.afferents[idx[i]].affclass]))
...
def plot_surface(obj, **args):
    ...
    if locator:
        ...
        dm = hv.DynamicMap(lambda x, y: hvobj*hv.Text(x, y+5, '(%d,%d)' %
                           tuple(obj.pixel2hand(np.array([x, y])))), streams=[pointer])
        return dm
